...
3. **Add close parenthesis:**
   ...
   - This ensures the parentheses remain balanced at every step so that a close parenthesis will never be placed before an open parenthesis.
...
|--------|-----------|-------------|
| **Time** | O(4^n / √n) | The number of valid parentheses sequences is the n-th Catalan number (1, 2, 5, 14, 42, 132, 429, 1430, 4862), which grows as ~4^n / (n^(3/2) √π). Each valid sequence requires O(n) time to build. Time complexity = 4^n / (n^(3/2) √π) × n ~ 4^n / √n (√π is a constant factor dominated by n^(3/2))|
