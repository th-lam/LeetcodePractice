...
2. **Record the current subset**
   - At the beginning of each recursive step, append a shallow copy (`path[:]`) to `res`. This can include the empty subset `[]` and all combinations.
...
4. **Backtrack**
   - After the recursive call returns, pop the last element from `path` to restore the state for the next iteration (loop).

## Complexity 
|   | Complexity | Explanation |
|--------|-----------|-------------|
| **Time** | O(n × 2^n) | There are 2^n total subsets for an array up to length n. Recording each subset requires copying a list of length n. |
